...
**Current state:** _Phase 0 scaffold + most of Phase 1 landed._ Tauri+React+TS app wired to `sharedcorelib` (vault, tiers, gating, reminders sweep, ice helpers, env, ui, common masters). Migrations 0001–0008. Working: **Today** (water + daily tasks), **Profiles** (self+family), **Vitals** logging, **Goals** (deterministic ETA projection — `domain/goals.ts`), **Reminders** inbox (bucketed, done/snooze/dismiss + manual add), Tracker-tier **Schedule**, Tracker-tier **Medications** (feed daily derived reminders), Caretaker-tier **Medical ICE card** (`pages/Ice.tsx`: allergies/conditions/meds/emergency contact via shared `ice` helpers, printable), **Journey** (tier ladder), the progressive-disclosure shell (Open/Nudge/Hidden + `FeatureGuard`), and the **habit reminder sweep** (`lib/reminderSweep.ts` → one OS notification per sweep, fired on idle). `npm run build` + `npm run test` (8 tests) are green. Not yet built: trends/import/directory pages (gated placeholders), OTA masters, document vault UI, doctor-visit report, the import pipeline. Update this line as phases land (see `docs/PLAN.md` §6).
