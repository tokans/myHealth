...
- **Yoga** (Tracker tier) — guided sequences with step-by-step poses + illustrations. A few samples ship in the app; richer **sequence bundles download separately** as signed packs from the project's GitHub release (receive-only, verified with the shared masters/OTA engine). Movement guidance only, not medical advice
- **Documents = scan & import** — one encrypted vault that doubles as the importer (no separate "Import" feature). Adding a document can **scan & extract** it: native-text passes straight through, images/PDFs accept pasted text until an on-device OCR sidecar lands, then a confidence-tiered, human-in-the-loop review structures it. The deterministic field-extraction layer (recognized text → structured drug/lab fields, with fuzzy formulary/LOINC matching, OCR-noise tolerance, and auto/disambiguate/manual tiering) lives in `src/import/`; safety-critical fields (drug, dosage) stay confirm-required
