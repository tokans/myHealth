...
- **Per-tab Excel import/export** — Reminders, Goals, Schedule, Medications and Vitals each have their own **Export / Import** buttons that read/write a small, hand-editable spreadsheet (person names not IDs, `HH:MM` times, "Every day", "Lower is better", Yes/No). Edit it offline and re-import — each row keeps an ID, so existing items update and new rows are added. On-device only; nothing is uploaded
