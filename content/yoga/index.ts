/**
 * Yoga — a baked content type (one tab). Ships a few sample sequences with inline
 * SVG pose pics so the tab works fully offline; richer sequences arrive as signed
 * downloadable bundles from the release tag below (see `bundles/` + the daily
 * content sync). Movement guidance only, not medical advice.
 */
