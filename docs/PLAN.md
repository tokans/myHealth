...
- **Image normalization + OCR + region/handwriting classification** (the heavy, model-bearing stages): the **free baseline is now Tesseract in the webview** (`@scandoc/core/ocr`, WASM — no native process; English language data downloaded once + SHA-256-verified + cached offline; PDFs rasterized via pdf.js). The heavier PaddleOCR/VLM pipeline remains a future **bundled local sidecar** for advanced/handwriting cases.
- **Domain-constrained field extraction + confidence tiering + review UI + correction logging**: **TS** in the webview, because it's UI + deterministic vocab matching, and it must own the human-in-the-loop gating. _(STATUS: the deterministic core is **implemented** — `extractPrescription`/`extractLab`/`parseDocument` + formulary + LOINC vocab (health-specific, stay in `src/import/`), over the domain-agnostic reading engine — fuzzy matching, OCR-noise normalization, auto/disambiguate/manual tiering, confirm-required drug/dosage — now extracted to the shared **`@scandoc/core`** package (`sharedCoreLib/scandoc-lib`) so myDocs reuses it. Covered by tests against difficult OCR fixtures (extractors in myHealth; engine primitives in the lib). The review UI and correction logging are still pending.)_
  - **Recognition seam:** `@scandoc/core` exposes a typed `Recognizer` interface (`recognize(CaptureInput) → RecognizedDoc`). Two implementations ship: the offline `nativeTextRecognizer` (native-text/plain-text fast path) and **`createTesseractRecognizer` (`@scandoc/core/ocr`)** for image/scanned-PDF bytes. A future Python OCR sidecar (§4.2) would be a third drop-in. The app injects the heavy peerDeps (`tesseract.js`/`pdfjs-dist`) and all I/O, so the seam stays bundler/host-agnostic.

### 4.2 Free OCR (shipped) + the future heavy sidecar
**Shipped (free, on-device):** `@scandoc/core/ocr` runs **Tesseract via `tesseract.js`** (WASM, in the webview — no native sidecar). The tesseract worker + core wasm + pdf.js worker are app-bundled (`public/ocr/`, generated by `scripts/build-ocr-assets.mjs`); only the ~10 MB `eng.traineddata.gz` is downloaded once (Tauri HTTP), **SHA-256-verified before write**, cached under `AppLocalData/ocr/`, and reused offline. Scanned PDFs are rasterized with pdf.js. Output stays `source:"ocr"` (confirm-required). Double-gated (`isTauri()` + `VITE_OCR_ASSET_BASE_URL`/`VITE_OCR_TRAINEDDATA_SHA256`); a one-time user consent precedes the download. This is the free deterministic floor; the paid **OpenMed** AI extraction is separate.

**Future (advanced) — bundled Python sidecar (decision to confirm)**
