...
- **Domain-constrained field extraction + confidence tiering + review UI + correction logging**: **TS** in the webview (`src/import/`), because it's UI + deterministic vocab matching, and it must own the human-in-the-loop gating. _(STATUS: the deterministic core is **implemented** — `extractPrescription`/`extractLab`/`parseDocument`, formulary + LOINC vocab, fuzzy matching, OCR-noise normalization, auto/disambiguate/manual tiering, confirm-required drug/dosage — covered by tests against difficult OCR fixtures. The review UI and correction logging are still pending.)_
