 ...
 *
 * myHealth's only sensitive-category surface is the document vault & import, which can
 * hold estate-adjacent paperwork (advance directives, insurance, legal-ish records) —
 * tagged `estate` so a `child_user` does not get the document/import affordance. Everything
 * else (metrics, goals, meds, schedule, ICE, trends) is medical and stays open.
 */
