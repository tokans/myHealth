...
describe("child-soft member policy (decision 19, UI-soft)", () => {
  it("owner / free single user is allowed every gate (additive: unchanged)", () => {
    ...
    expect(memberPolicy.isFeatureAllowed(undefined, "documents", GATE_CATEGORIES.documents)).toBe(true);
  });

  it("child_user is denied the sensitive-category gates (documents → estate)", () => {
